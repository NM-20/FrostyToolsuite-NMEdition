...
public class ShaderPermutation : IDisposable {
  ...
  public bool LoadShaders(ID3D11Device device) {
    ...
    for (int i = 0; i < geomDecl.ElementCount; i++) {
      ...
      case VertexElementFormat.Half2:
        format = Vortice.DXGI.Format.R16G16_Float;
        break;
      ...
      case VertexElementFormat.UByteN:
        format = Vortice.DXGI.Format.R8_UInt;
        break;
      case VertexElementFormat.Byte4:
        format = Vortice.DXGI.Format.R8G8B8A8_SInt;
        break;
      case VertexElementFormat.Byte4N:
        format = Vortice.DXGI.Format.R8G8B8A8_SNorm;
        break;
      case VertexElementFormat.UByte4N:
        format = Vortice.DXGI.Format.R8G8B8A8_UNorm;
        break;
      case VertexElementFormat.UByte4:
        format = Vortice.DXGI.Format.R8G8B8A8_UInt;
        break;

      case VertexElementFormat.Short2N:
        format = Vortice.DXGI.Format.R16G16_SNorm;
        break;
      case VertexElementFormat.UShort4:
        format = Vortice.DXGI.Format.R16G16B16A16_UInt;
        break;
      case VertexElementFormat.Short4:
        format = Vortice.DXGI.Format.R16G16B16A16_SInt;
        break;
      case VertexElementFormat.Short4N:
        format = Vortice.DXGI.Format.R16G16B16A16_SNorm;
        break;
  ...
  public unsafe void AssignFallbackParameters(RenderCreateState state, /*MeshRenderSection*/ dynamic section, MeshMaterial material) {
    ...
    using (NativeWriter writer = new NativeWriter(new MemoryStream())) {
      ...
      writer.Write(0.0f);
      writer.Write(0.0f);
      writer.Write(TintColorA.X);
      writer.Write(TintColorA.Y);
      writer.Write(TintColorA.Z);
      writer.Write(TintColorA.W);
      writer.Write(TintColorB.X);
      writer.Write(TintColorB.Y);
      writer.Write(TintColorB.Z);
      writer.Write(TintColorB.W);
      writer.Write(TintColorC.X);
      writer.Write(TintColorC.Y);
      writer.Write(TintColorC.Z);
      writer.Write(TintColorC.W);
      writer.Write(TintColorD.X);
      writer.Write(TintColorD.Y);
      writer.Write(TintColorD.Z);
      writer.Write(TintColorD.W);
      ...
      foreach (XmlNode childNode in permNodes) {
        ...
        if (samplersNode != null) {
          foreach (XmlNode samplerNode in samplersNode) {
            ...
            if (samplerNode.Attributes["filter"] != null)
              filter = (Filter)Enum.Parse(typeof(Filter), samplerNode.Attributes["filter"].Value);
            if (samplerNode.Attributes["addressU"] != null)
              addressU = (TextureAddressMode)Enum.Parse(typeof(TextureAddressMode), samplerNode.Attributes["addressU"].Value);
            if (samplerNode.Attributes["addressV"] != null)
              addressV = (TextureAddressMode)Enum.Parse(typeof(TextureAddressMode), samplerNode.Attributes["addressV"].Value);
            if (samplerNode.Attributes["addressW"] != null)
              addressW = (TextureAddressMode)Enum.Parse(typeof(TextureAddressMode), samplerNode.Attributes["addressW"].Value);
            if (samplerNode.Attributes["mipLodBias"] != null)
              mipLodBias = float.Parse(samplerNode.Attributes["mipLodBias"].Value, ci);
            if (samplerNode.Attributes["maxAnisotropy"] != null)
              maxAniso = int.Parse(samplerNode.Attributes["maxAnisotropy"].Value);
            if (samplerNode.Attributes["minLod"] != null)
              minLod = float.Parse(samplerNode.Attributes["minLod"].Value, ci);
            if (samplerNode.Attributes["maxLod"] != null)
              maxLod = float.Parse(samplerNode.Attributes["maxLod"].Value, ci);
    ...
    StringBuilder sb = new StringBuilder();
    {
      ...
      foreach (ShaderParameter param in permutation.PixelTextures) {
        ...
        case ShaderParameterType.Tex2d:
          ...
          break;
        case ShaderParameterType.Tex2dArray:
          textureFormat = "Texture2DArray";
          break;
        case ShaderParameterType.TexCube:
...
public class MeshMaterialCollection : IEnumerable<MeshMaterial> {
  ...
  private void PopulateMaterialFromShaderInstance(MeshMaterial material, dynamic /* SurfaceShaderInstanceDataStruct */ shader, out PointerRef shaderRef) {
    ...
    foreach (dynamic param in shader.BoolParameters) {
      ...
      if (idx != -1)
        material.BoolParameters[idx] = param;
      else
        material.BoolParameters.Add(param);
    }
    foreach (dynamic param in shader.VectorParameters) {
      ...
      if (idx != -1)
        material.VectorParameters[idx] = param;
      else
        material.VectorParameters.Add(param);
    }
    foreach (dynamic param in shader.TextureParameters) {
      ...
      if (idx != -1)
        material.TextureParameters[idx] = param;
      else
        material.TextureParameters.Add(param);
    }
    if (ProfilesLibrary.DataVersion == (int)ProfileVersion.StarWarsBattlefrontII || ProfilesLibrary.DataVersion == (int)ProfileVersion.Anthem || ProfilesLibrary.DataVersion == (int)ProfileVersion.StarWarsSquadrons) {
      foreach (dynamic param in shader.ConditionalParameters) {
        ...
        if (idx != -1)
          material.ConditionalParameters[idx] = param;
        else
          material.ConditionalParameters.Add(param);
      }
    }
  }
