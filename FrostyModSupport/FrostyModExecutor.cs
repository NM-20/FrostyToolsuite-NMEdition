...
public partial class FrostyModExecutor {
  ...
  private void ProcessModResources(IResourceContainer fmod) {
    ...
    Parallel.ForEach(fmod.Resources, new ParallelOptions() { MaxDegreeOfParallelism = Environment.ProcessorCount }, resource => {
      ...
      foreach (int bundleHash in bundles) {
        ...
        case ModResourceType.Ebx:
          modBundle.Modify.AddEbx(resource.Name);
          break;
        case ModResourceType.Res:
          modBundle.Modify.AddRes(resource.Name);
          break;
    ...
    label_add_bundles:
      ...
      foreach (int bundleHash in resource.AddedBundles) {
        ...
        switch (resource.Type) {
        case ModResourceType.Ebx:
          modBundle.Add.AddEbx(resource.Name);
          break;
        case ModResourceType.Res:
          modBundle.Add.AddRes(resource.Name);
          break;
        case ModResourceType.Chunk:
  ...
  private void ProcessLegacyModResources(string modPath) {
    ...
    foreach (DbObject action in mod.GetValue<DbObject>("actions")) {
      ...
      if (actionType == "modify") {
        switch (resType) {
        case "ebx":
          modBundle.Modify.Ebx.Add(resName);
          break;
        case "res":
          modBundle.Modify.Res.Add(resName);
          break;
        case "chunk":
          modBundle.Modify.Chunks.Add(new Guid(resName));
          break;
        }
      }
        ...
        switch (resType) {
        case "ebx":
          modBundle.Add.Ebx.Add(resName);
          break;
        case "res":
          modBundle.Add.Res.Add(resName);
          break;
        case "chunk":
          modBundle.Add.Chunks.Add(new Guid(resName));
          break;
        }
      }
        ...
        switch (resType) {
        case "ebx":
          modBundle.Remove.Ebx.Add(resName);
          break;
        case "res":
          modBundle.Remove.Res.Add(resName);
          break;
        case "chunk":
          modBundle.Remove.Chunks.Add(new Guid(resName));
          break;
        }
      }
    }
  ...
  private void WriteArchiveData(string catalog, CasDataEntry casDataEntry) {
    ...
    foreach (Sha1 sha1 in casDataEntry.EnumerateDataRefs()) {
      ...
      case "1GB":
        casMaxBytes = 1073741824;
        break;
      case "512MB":
        casMaxBytes = 536870912;
        break;
      case "256MB":
        casMaxBytes = 268435456;
        break;
      }
