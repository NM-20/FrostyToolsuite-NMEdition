...
public class ModPrimaryActionTooltipConverter : IValueConverter {
  public object Convert(object value, Type targetType, object parameter, CultureInfo culture) {
    ...
    if (index != -1) {
      ...
      case ModPrimaryActionType.None:
        return null;
      case ModPrimaryActionType.Modify:
        return (index == mri.FirstModToModifyIndex) ? "Resource is initially modified by this mod" : "Resource is replaced by this mod";
      case ModPrimaryActionType.Add:
        return (index == mri.FirstModToModifyIndex) ? "Resource is initially added by this mod" : "Resource is replaced by this mod";
      case ModPrimaryActionType.Merge:
        return (index == mri.FirstModToModifyIndex) ? "Resource is initially modified by this mod" : "Resource is merged by this mod";
      }
    }
  ...
  public object Convert(object value, Type targetType, object parameter, CultureInfo culture) {
    ...
    if (index != -1) {
      ...
      case ModPrimaryActionType.None:
        return blankSource;
      case ModPrimaryActionType.Modify:
        return (index == mri.FirstModToModifyIndex) ? primaryActionModifySource : primaryActionReplaceSource;
      case ModPrimaryActionType.Add:
        return (index == mri.FirstModToModifyIndex) ? primaryActionAddSource : primaryActionReplaceSource;
      case ModPrimaryActionType.Merge:
        return (index == mri.FirstModToModifyIndex) ? primaryActionModifySource : primaryActionMergeSource;
      }
    }
      ...
      foreach (string filename in filenames) {
        ...
        try {
          if (IsCompressed(fi)) {
            ...
            if (fi.Extension == ".rar")
              decompressor = new RarDecompressor();
            else if (fi.Extension == ".zip" || fi.Extension == ".fbpack")
              decompressor = new ZipDecompressor();
            else if (fi.Extension == ".7z")
              decompressor = new SevenZipDecompressor();
                ...
                if (resource.HasHandler) {
                  if ((uint)resource.Handler == 0xBD9BFB65)
                    ...
                    if (handler.Usage == HandlerUsage.Merge) {
                      ...
                      primaryAction = ModPrimaryActionType.Modify;
                  }
                }
                else if (resource.IsAdded)
                  primaryAction = ModPrimaryActionType.Add;
