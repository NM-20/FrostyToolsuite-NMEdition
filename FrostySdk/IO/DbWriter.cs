...
public class DbWriter : NativeWriter {
  ...
  public byte[] WriteDbObject(string name, object inObj) {
    ...
    using (NativeWriter writer = new NativeWriter(ms)) {
      ...
      case DbType.Boolean:
        writer.Write((byte)((bool)inObj ? 0x01 : 0x00));
        break;
      case DbType.String:
        writer.WriteSizedString(((string)inObj) + "\0");
        break;
      case DbType.Int:
        writer.Write((int)inObj);
        break;
      case DbType.Long:
        writer.Write((long)inObj);
        break;
      case DbType.Float:
        writer.Write((float)inObj);
        break;
      case DbType.Double:
        writer.Write((double)inObj);
        break;
      case DbType.Guid:
        writer.Write((Guid)inObj);
        break;
      case DbType.Sha1:
        writer.Write((Sha1)inObj);
        break;
