  ...
  public static class TypeLibrary {
          ...
          if (line.StartsWith("BeginClass")) {
            ...
            while (!line.StartsWith("EndClass")) {
              ...
              if (line.StartsWith("BeginFields")) {
                ...
                while (!line.StartsWith("EndFields")) {
                  ...
                  if (line.StartsWith("BeginField ")) {
                    ...
                    while (!line.EndsWith("EndField")) {
                      ...
                      if (arr[0].StartsWith("DisplayName"))
                        fieldObj.SetValue("displayName", arr[1]);
                      else if (arr[0].StartsWith("Description"))
                        fieldObj.SetValue("description", arr[1]);
                      else if (arr[0].StartsWith("Category"))
                        fieldObj.SetValue("category", arr[1]);
                      else if (arr[0].StartsWith("Editor"))
                        fieldObj.SetValue("editor", arr[1]);
                      else if (arr[0].StartsWith("Transient"))
                        fieldObj.SetValue("transient", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("ReadOnly"))
                        fieldObj.SetValue("readOnly", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("Reference"))
                        fieldObj.SetValue("reference", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("Added"))
                        fieldObj.SetValue("added", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("Hidden"))
                        fieldObj.SetValue("hidden", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("Index"))
                        fieldObj.SetValue("index", int.Parse(arr[1]));
                      else if (arr[0].StartsWith("HideChildren"))
                        fieldObj.SetValue("hideChildren", bool.Parse(arr[1]));
                      else if (arr[0].StartsWith("InterfaceType")) {
                        int it = int.Parse(arr[1]);
                        switch (it) {
                        case 0:
                          fieldObj.SetValue("property", 0);
                          break;
                        case 1:
                          fieldObj.SetValue("property", 1);
                          break;
                        case 2:
                          fieldObj.SetValue("event", 0);
                          break;
                        case 3:
                          fieldObj.SetValue("event", 1);
                          break;
                        case 4:
                          fieldObj.SetValue("link", 0);
                          break;
                        case 5:
                          fieldObj.SetValue("link", 1);
                          break;
                        }
                      }
                ...
                string[] arr = SplitOnce(line);
                if (arr[0].StartsWith("DisplayName"))
                  classObj.SetValue("displayName", arr[1]);
                else if (arr[0].StartsWith("Description"))
                  classObj.SetValue("description", arr[1]);
                else if (arr[0].StartsWith("ValueConverter"))
                  classObj.SetValue("valueConverter", arr[1]);
                else if (arr[0].StartsWith("Alignment"))
                  classObj.SetValue("alignment", int.Parse(arr[1]));
                else if (arr[0].StartsWith("Abstract"))
                  classObj.SetValue("abstract", bool.Parse(arr[1]));
                else if (arr[0].StartsWith("Inline"))
                  classObj.SetValue("inline", bool.Parse(arr[1]));
                else if (arr[0].StartsWith("Icon"))
                  classObj.SetValue("icon", arr[1]);
                else if (arr[0].StartsWith("Realm"))
                  classObj.SetValue("realm", arr[1]);
              }
            }
    ...
    private static Type GetTypeFromEbxType(EbxFieldType inType, string baseType, int arrayType = -1) {
      ...
      case EbxFieldType.CString:
        type = typeof(Ebx.CString);
        break;
      case EbxFieldType.ResourceRef:
        type = typeof(Ebx.ResourceRef);
        break;
      case EbxFieldType.FileRef:
        type = typeof(Ebx.FileRef);
        break;
      case EbxFieldType.TypeRef:
        type = typeof(Ebx.TypeRef);
        break;
      case EbxFieldType.Array:
        type = typeof(List<>).MakeGenericType(GetTypeFromEbxType((EbxFieldType)((arrayType >> 4) & 0x1F), baseType));
        break;
    ...
    internal static dynamic CreateObject(Type inType) {
      ...
      if (attr != null) {
        ...
        case EbxFieldType.Boolean:
          ...
          break;
        case EbxFieldType.BoxedValueRef:
          inType = typeof(BoxedValueRef);
          break;
        case EbxFieldType.CString:
          inType = typeof(CString);
          break;
        case EbxFieldType.Float32:
          inType = typeof(float);
          break;
