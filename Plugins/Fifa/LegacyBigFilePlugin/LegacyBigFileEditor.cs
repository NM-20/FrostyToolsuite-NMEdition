...
public class IBXReader : NativeReader {
  ...
  public string ReadXml() {
    ...
    for (int i = 0; i < numProps; i++) {
      ...
      switch (type) {
      case 0xE0:
        val = Encoding.UTF8.GetBytes(strings[ReadUShort(Endian.Big)]);
        break;
      case 0xD0:
        val = Encoding.UTF8.GetBytes(strings[ReadByte()]);
        break;
      case 0xC0:
        val = Encoding.UTF8.GetBytes(strings[(b & 0x3F)]);
        break;
      case 0xB0:
        val = ReadBytes(4);
        break;
      case 0x40:
        val = new byte[] { (byte)(b & 0x0F) };
        break;
      case 0x30:
        val = ReadBytes(4);
        break;
...
public class LegacyBigFileEditor : FrostyAssetEditor {
  ...
  private void Reconstruct(AssetEntry assetEntry) {
    if (assetEntry.Type == "BIG")
      ReconstructBIG(assetEntry);
    else
      ReconstructAST(assetEntry);
  }
    ...
    using (NativeReader reader = new NativeReader(App.AssetManager.GetCustomAsset("legacy", assetEntry))) {
      using (NativeWriter writer = new NativeWriter(new MemoryStream())) {
        ...
        foreach (BigFileEntry bfe in lb.Items) {
          ...
          switch (bfe.Type) {
          case "DDS":
            typeIndex = 0x264923D3;
            break;
          case "UNK":
            typeIndex = 0x264923D2;
            break;
          case "APT":
            typeIndex = 0x264923D6;
            break;
          case "APTD":
            typeIndex = 0x264923D5;
            break;
          case "DAT":
            typeIndex = 0x8fe5b6d9;
            break;
          }
