  ...
  private void LoadKitAssets(FifaKitPropertyInfo info) {
    ...
    foreach (EbxAssetEntry entry in App.AssetManager.EnumerateEbx(type: "TextureAsset")) {
      if (entry.Name.Contains(jerseyPattern)) {
        if (entry.Name.EndsWith("color"))
          meshes[0].Item3.SetTextureParameter("colorTexture", entry);
        else if (entry.Name.EndsWith("normal"))
          meshes[0].Item3.SetTextureParameter("normalTexture", entry);
        else
          meshes[0].Item3.SetTextureParameter("coefficientTexture", entry);
      }
      else if (entry.Name.Contains(shortsPattern)) {
        if (entry.Name.EndsWith("color"))
          meshes[1].Item3.SetTextureParameter("colorTexture", entry);
        else if (entry.Name.EndsWith("normal"))
          meshes[1].Item3.SetTextureParameter("normalTexture", entry);
        else
          meshes[1].Item3.SetTextureParameter("coefficientTexture", entry);
      }
      else if (entry.Name.Contains(socksPattern)) {
        if (entry.Name.EndsWith("color"))
          meshes[2].Item3.SetTextureParameter("colorTexture", entry);
        else if (entry.Name.EndsWith("normal"))
          meshes[2].Item3.SetTextureParameter("normalTexture", entry);
        else
          meshes[2].Item3.SetTextureParameter("coefficientTexture", entry);
      }
